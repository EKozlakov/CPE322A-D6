...
def home(request):
    ...
    dtstate = '2022'
    r = requests.get('http://127.0.0.1:8000/dt/1/', auth=('az77x3_@-Y^-3', 'az77x3_@-Y^-3'))
    result = r.text
    output = json.loads(result)
    dtstate = output['name']

    memstate = '20'
    r = requests.get('http://127.0.0.1:8000/mem/1/', auth=('az77x3_@-Y^-3', 'az77x3_@-Y^-3'))
    result = r.text
    output = json.loads(result)
    memstate = output['name']

    cpustate = '20'
    r = requests.get('http://127.0.0.1:8000/cpu/1/', auth=('az77x3_@-Y^-3', 'az77x3_@-Y^-3'))
    result = r.text
    output = json.loads(result)
